...
\begin{enumerate}
	...
	\item An \textbf{electroencephalogram (EEG)}measure the electrical activity emitted from the brain's neurons and produces an amplified readout. The resulting EEG reveals the gross electrical activity of the brain. Researchers often introduce a specific stimulus to seek a particular response signal while having a computer filter out the unrelated noise. The resulting information is called the \textbf{sensory evoked potential} and is used to diagnose the cause of seizures clinically.
